 ...
 *
 * Allows users to top-up any ticket type (except entry tickets) by entering their ticket code.
...
(function () {
  ...
  async function lookupTicket() {
    ...
    try {
      ...
      // Entry tickets cannot be topped up
  ...
  function renderTicketInfo() {
    ...
    var typeLabels = {
      ...
      food: "Food Voucher",
      drinks: "Drinks Voucher",
      kids_zone: "Kids Zone",
    };
    $("topup-ticket-meta").textContent = typeLabels[ticket.type] || ticket.type;
    $("topup-current-balance").textContent =
      "D" + (ticket.balance || 0).toLocaleString();
  ...
  async function loadBundlesAndCap() {
    try {
      var [bundlesRes, capRes] = await Promise.all([
        fetch(
          SUPABASE_URL +
            "/rest/v1/top_up_bundles?is_active=eq.true&order=sort_order.asc",
          {
        ...
        fetch(
          SUPABASE_URL +
            "/rest/v1/system_config?key=eq.balance_cap&select=value",
          {
            headers: { apikey: SUPABASE_ANON_KEY },
          },
        ),
  ...
  function setupForm() {
    ...
    document.querySelectorAll("[data-payment]").forEach(function (el) {
      el.addEventListener("click", function () {
        ...
        if (waveDetails)
          waveDetails.classList.toggle("active", selectedPayment === "wave");
        updatePayButton();
      });
    });
  ...
  function selectBundle(idx) {
    ...
    document.querySelectorAll(".bundle-btn").forEach(function (b) {
  ...
  function updateSummary() {
    ...
    if (balanceAfter) {
      balanceAfter.innerHTML =
        "Balance after: <strong>D" + newBalance.toLocaleString() + "</strong>";
    }
    ...
    if (capCheck) {
      if (newBalance > balanceCap) {
        capCheck.textContent =
          "Exceeds max balance of D" + balanceCap.toLocaleString();
  ...
  async function handlePay() {
    ...
    if (newBalance > balanceCap) {
      errEl.textContent =
        "This would exceed the max balance of D" +
        balanceCap.toLocaleString() +
        ".";
      errEl.style.display = "block";
      return;
    }
  ...
  async function handleModemPay() {
    ...
    if (!payUrl || !payUrl.startsWith("http")) {
      throw new Error(
        "Payment gateway did not return a valid checkout URL. Please try again or use Wave Transfer.",
      );
    }
  ...
  async function handleWave() {
    ...
    $("topup-form-container").parentNode.insertBefore(
      conf,
      $("topup-form-container").nextSibling,
    );
  ...
  async function handleTopupReturn(pending) {
    ...
    showView("loading");
    $("topup-loading").querySelector("p").textContent =
      "Verifying payment\u2026";
    ...
    banner.style.cssText =
      "background:var(--accent-dim);border:1px solid var(--accent);border-radius:12px;padding:16px;margin-bottom:16px;text-align:center;";
    banner.innerHTML =
      '<div style="font-size:28px;margin-bottom:8px;">\uD83D\uDCE7</div>' +
      '<p style="font-size:14px;font-weight:500;margin:0 0 4px;">Payment Received — Still Processing</p>' +
      '<p style="font-size:13px;color:var(--muted);margin:0 0 12px;">Your top-up of <strong>D' +
      pending.amount.toLocaleString() +
      "</strong> was successful but we\u2019re still applying the credits. This usually takes just a moment.</p>" +
