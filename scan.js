...
(function () {
  ...
  const SUPA_URL =
    typeof SUPABASE_URL !== "undefined"
      ? SUPABASE_URL
      : "https://anigcqdquakinlzvyaur.supabase.co";
  const SUPA_ANON =
  ...
  function initPasscode() {
    ...
    var submitBtn = document.querySelector(
      "#scanner-passcode .btn, #passcode-input + button",
    );
    if (!submitBtn) {
  ...
  function validatePasscode() {
    ...
    if (!code) {
      if (errorEl) {
        errorEl.textContent = "Please enter your staff code.";
    ...
    callEdgeFunction("/staff-auth", { code: code })
      ...
      .catch(function (err) {
        if (errorEl) {
          errorEl.textContent =
            err.message || "Invalid staff code. Please try again.";
          errorEl.style.display = "block";
        }
        if (btn) {
          btn.disabled = false;
...
function lockScanner() {
    ...
    try {
      sessionStorage.removeItem("wf_scanner_session");
    } catch (e) {}
    stopCamera();
    resetScannerView();
  ...
  function selectMode(mode) {
    if (state.billClearTimer) {
      clearTimeout(state.billClearTimer);
      state.billClearTimer = null;
    }
    if (state.billReceiptDelay) {
      clearTimeout(state.billReceiptDelay);
      state.billReceiptDelay = null;
    }
    state.mode = mode;
    ...
    if (mode === "gate") {
      modeLabel = "Gate";
      title.textContent = "Gate — Verify Entry";
    } else if (mode === "debit") {
      modeLabel = "Debit";
      title.textContent = "Debit — Activity Credits";
    } else if (mode === "topup") {
      modeLabel = "Top-Up";
    ...
    state.barcodeDetector
      ...
      .then(function (barcodes) {
        if (barcodes.length > 0 && state.scanningActive) {
          ...
          if (
            code &&
            code !== (state.currentTicket ? state.currentTicket.code : null)
          ) {
  ...
  function manualLookup() {
    ...
    if (!code) {
      showError("scanner-result", "Please enter a ticket code.");
      return;
    }
  ...
  function toggleEmailLookup() {
    ...
    emailLookup.style.display =
      emailLookup.style.display === "none" ? "" : "none";
  ...
  function searchByEmail() {
    ...
    var mode = state.mode;
    callEdgeFunction("/lookup-by-email", {
      ...
      .then(function (data) {
        ...
        if (!tickets || tickets.length === 0) {
          if (results)
            results.innerHTML =
              '<p style="font-size:13px;color:var(--muted);padding:12px 0;">No active tickets found for this email.</p>';
          return;
        }
        var html = "";
        tickets.forEach(function (t) {
          ...
          html +=
            '<div class="email-result-item" data-code="' +
            escapeHtml(t.code) +
            '">' +
            '<div><div class="code">' +
            escapeHtml(t.code) +
      ...
      .catch(function (err) {
        if (results)
          results.innerHTML =
            '<p style="font-size:13px;color:#c53030;padding:12px 0;">' +
    ...
    callEdgeFunction("/lookup-ticket", { code: normalizedCode })
      .then(function (data) {
        if (!data.success || !data.ticket) {
          showResult(
            "error",
            "Ticket not found: " + escapeHtml(normalizedCode),
          );
          if (ticketResult) ticketResult.innerHTML = "";
          return;
        }
  ...
  function renderScannerTicket(ticket) {
    ...
    var typeName = ticket.ticket_type
    ...
    if (ticket.status === "active") {
      ...
      statusLabel = "Used";
      statusClass = "used";
    } else if (ticket.status === "exhausted") {
    ...
    container.innerHTML =
      '<div class="scanner-ticket-card" data-ticket-id="' +
  ...
  function generateBillReceiptHTML(ticket) {
    ...
    return (
      ...
      voucherEmoji +
      "</div>" +
      '<div style="font-size:24px;font-weight:700;letter-spacing:0.06em;font-family:monospace;word-break:break-all;">' +
      escapeHtml(ticket.code) +
      "</div>" +
  ...
  function printBillReceipt() {
    ...
    var ticketSnapshot = {
      ...
      customer_name: state.currentTicket
        ? state.currentTicket.customer_name
        : "",
    };
    ...
    content.innerHTML =
      ...
      voucherLabel +
      "</div>" +
      '<div style="font-size:22px;font-weight:700;letter-spacing:0.12em;font-family:monospace;margin:6px 0;">' +
      escapeHtml(ticketSnapshot.code) +
      "</div>" +
      "</div>" +
      '<table style="width:100%;font-size:11px;line-height:1.6;">' +
      '<tr><td style="padding:1px 0;color:#555;">Voucher</td><td style="padding:1px 0;text-align:right;">' +
      voucherLabel +
      "</td></tr>" +
      ...
      "</td></tr>" +
      '<tr><td style="padding:1px 0;color:#555;">Date</td><td style="padding:1px 0;text-align:right;">' +
      dateStr +
      "</td></tr>" +
      '<tr><td style="padding:1px 0;color:#555;">Time</td><td style="padding:1px 0;text-align:right;">' +
      timeStr +
      "</td></tr>" +
      ...
      '<tr><td style="padding:1px 0;color:#555;">Staff</td><td style="padding:1px 0;text-align:right;">' +
      escapeHtml(state.scannerCode || "—") +
      "</td></tr>" +
      "</table>" +
  ...
  function toggleBillLog() {
    ...
    if (logPanel)
      logPanel.classList.toggle("active", state.billLogPanelVisible);
  }
  ...
  function renderBillLog() {
    ...
    var foodCount = state.billLog.filter(function (e) {
      return e.type === "food";
    }).length;
    var drinksCount = state.billLog.filter(function (e) {
      return e.type === "drinks";
    }).length;
    var totalValue = state.billLog.reduce(function (sum, e) {
      return sum + (e.value || 0);
    }, 0);

    var html =
      '<div class="bill-log-summary">' +
      "<span>Total: <strong>" +
      total +
      "</strong></span>" +
      "<span>" +
    ...
    logCopy.forEach(function (entry, idx) {
      ...
      if (state.undoingEntryIdx === originalIdx) {
        ...
        html +=
          '<div class="bill-log-item undoing" data-log-idx="' +
          originalIdx +
          '">' +
          '<div class="undo-note-wrapper">' +
          '<div class="undo-note-label">Why undo <strong>' +
          ...
          '<button class="btn btn-primary undo-note-confirm" data-log-idx="' +
          originalIdx +
          '" style="flex:1;padding:10px;font-size:13px;">Confirm Undo</button>' +
        ...
        html +=
          '<div class="bill-log-item' +
          (idx === 0 ? " latest" : "") +
          '" data-log-idx="' +
          originalIdx +
          '">' +
          '<div class="bill-log-item-icon">' +
          emoji +
          "</div>" +
          '<div class="bill-log-item-info">' +
          '<div class="bill-log-item-code">' +
          escapeHtml(entry.code) +
          "</div>" +
          '<div class="bill-log-item-customer">' +
  ...
  function exportBillLog() {
    if (state.billLog.length === 0) {
      showResult(
        "info",
        "Nothing to export — no vouchers redeemed this session.",
      );
      return;
    }
    ...
    var totals = state.billLog.reduce(
      function (acc, e) {
        acc.totalCount++;
  ...
  function confirmUndoWithNote(idx) {
    ...
    var confirmBtn = document.querySelector(
      '.undo-note-confirm[data-log-idx="' + idx + '"]',
    );
    if (confirmBtn) {
  ...
  function handleUndoRedemption(idx, reason) {
    ...
    var undoBtn = document.querySelector(
      '.bill-log-undo-btn[data-log-idx="' + idx + '"]',
    );
    if (undoBtn) {
  ...
  function showBillActions(ticket, container) {
    ...
    container.innerHTML =
      ...
      "</div>" +
      "<div>" +
      '<div style="font-size:13px;color:var(--accent-text);font-weight:600;">' +
      voucherLabel +
      "</div>" +
      '<div style="font-size:12px;color:var(--muted);">' +
      "Balance: <strong>" +
  ...
  function markTicketUsed(ticketId) {
    ...
    callEdgeFunction("/mark-used", { ticket_id: ticketId })
      .then(function (data) {
        if (!data.success)
          throw new Error(data.error || "Failed to mark ticket");
        ...
        if (state.mode === "bill") {
          ...
          state.billLog.push({
            ...
            value:
              state.currentTicket.type === "food"
                ? 200
                : state.currentTicket.type === "drinks"
                  ? 150
                  : 0,
          });
          ...
          state.billReceiptDelay = setTimeout(function () {
            ...
            state.billClearTimer = setTimeout(function () {
              ...
              var ticketResult = document.getElementById(
                "scanner-ticket-result",
              );
              if (ticketResult) ticketResult.innerHTML = "";
              redeemedTicket = null;
      ...
      .catch(function (err) {
        showError(
          "gate-error",
          err.message || "Failed to mark ticket as used.",
        );
        if (btn) {
          btn.disabled = false;
          btn.textContent =
            state.mode === "bill" ? "Redeem Voucher" : "Mark as Entered";
        }
      });
  }
  ...
  function loadBoothBundles() {
    ...
    supabaseGet(
      "/rest/v1/top_up_bundles?select=id,amount,is_active,sort_order&is_active=eq.true&order=sort_order.asc",
    )
      .then(function (bundles) {
        state.bundles = (bundles || []).map(function (b, i) {
          b._idx = i;
          return b;
        });
        renderBoothBundles();
    ...
    supabaseGet(
      "/rest/v1/system_config?select=key,value&key=eq.balance_cap&limit=1",
    )
  ...
  function renderBoothBundles() {
    ...
    state.bundles.forEach(function (b) {
      ...
      html +=
        '<button class="bundle-btn booth-bundle-btn' +
        selected +
        '" data-idx="' +
        b._idx +
        '">' +
  ...
  function showTopupForm(ticket) {
    if (ticket.type === "entry") {
      ...
      if (actions) {
        actions.innerHTML =
          '<div style="padding:16px;text-align:center;color:var(--muted);font-size:14px;">' +
          "Entry tickets cannot be topped up. All other ticket types are eligible." +
          "</div>";
      }
      return;
    }
  ...
  function getBoothAmount() {
    if (
      ...
      state.selectedBundleIdx < state.bundles.length
    ) {
      ...
      if (newBalance > state.balanceCap) {
        showResult(
          ...
          "This top-up would exceed the maximum balance of " +
            formatCurrency(state.balanceCap) +
            ".",
        );
      }
    }
  ...
  function handlePaymentMethod(method) {
    ...
    if (!state.currentTicket) {
      showResult(
        "error",
        "No ticket selected. Scan or enter a ticket code first.",
      );
      return;
    }
    ...
    if (newBalance > state.balanceCap) {
      showResult(
        "error",
        "Would exceed max balance of " + formatCurrency(state.balanceCap),
      );
      return;
    }
  ...
  function handleBoothModemPay(ticket, amount) {
    ...
    var paymentUrl =
      window.location.origin + "/top-up?t=" + ticket.code + "&amount=" + amount;
    ...
    callEdgeFunction("/create-order", {
      ...
      .then(function (orderData) {
        ...
        if (qrStatus)
          qrStatus.textContent = "Order created. Polling for payment…";
          ...
          callEdgeFunction("/check-order", { order_id: orderId })
            .then(function (data) {
              if (data.success && data.status === "paid") {
                ...
                if (qrStatus)
                  qrStatus.textContent = "Payment confirmed! Processing…";
                ...
                callEdgeFunction("/lookup-ticket", { code: ticket.code })
                  .then(function (lookupData) {
                    if (lookupData.success && lookupData.ticket) {
                      ...
                      if (state.currentTicket)
                        state.currentTicket.balance = newBalance;
                      renderScannerTicket(state.currentTicket);
                    }
                  })
                  .catch(function () {});
  ...
  function handleBoothModemPayOverride() {
    ...
    processBoothTopup(
      ...
      amount,
      "modempay",
      "Booth — manual override (ModemPay)",
    );
  }
    ...
    callEdgeFunction("/create-order", {
      ...
      .then(function (data) {
        if (!data || !data.success)
          throw new Error(
            (data && data.error) || "Payment confirmation failed",
          );

        var newBalance =
          data.new_balance !== null && data.new_balance !== undefined
            ? data.new_balance
            : (ticket.balance || 0) + amount;
        // Update local state
  ...
  function toggleCrossType(mode) {
    ...
    $$(".toggle-btn[data-ct]").forEach(function (b) {
      b.classList.toggle("active", b.getAttribute("data-ct") === mode);
    });
    ...
    supabaseGet(
      ...
      .then(function (types) {
        if (!types || types.length === 0) {
          select.innerHTML =
            '<option value="">No activity credits available</option>';
          return;
        }
        var html = types
          .map(function (t) {
  ...
  function createNewTicket() {
    ...
    callEdgeFunction("/create-order", {
      ...
      .catch(function (err) {
        if (errorEl) {
          errorEl.textContent = err.message || "Failed to create ticket.";
          errorEl.style.display = "block";
        }
        if (payBtn) {
          payBtn.disabled = false;
  ...
  function submitBulk() {
    ...
    if (!rows || rows.length === 0) {
      if (resultEl) {
        ...
        resultEl.innerHTML = "No rows to submit.";
    ...
    rows.forEach(function (tr) {
      ...
      var normalizedCode = codeVal.startsWith("TKT-")
        ? codeVal
        : "TKT-" + codeVal;
      entries.push({
        code: normalizedCode,
    ...
    if (entries.length === 0) {
      if (resultEl) {
        resultEl.className = "scanner-result error";
        resultEl.innerHTML = "No valid entries found.";
        resultEl.style.display = "block";
      }
      return;
    }
    ...
    if (submitBtn) {
      ...
      submitBtn.textContent = "Processing " + entries.length + "…";
    }
    ...
    callEdgeFunction("/bulk-topup", { entries: entries })
      ...
      .catch(function (err) {
        if (resultEl) {
          resultEl.className = "scanner-result error";
          resultEl.innerHTML =
            err.message || "Bulk top-up failed. Please try again.";
          resultEl.style.display = "block";
        }
      })
      .finally(function () {
  ...
  function initEventDelegation() {
    document.addEventListener("click", function (e) {
      ...
      if (target.closest("#debit-confirm-btn")) {
        ...
        if (!amount || amount < 0) {
          showError("debit-error", "Please enter a valid amount.");
          return;
        }
